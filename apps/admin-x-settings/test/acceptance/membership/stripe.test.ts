...
test.describe('Stripe settings', async () => {
    test('Supports the Stripe Connect flow', async ({page}) => {
        const {lastApiRequests} = await mockApi({page, requests: {
            ...
            editSettings: {method: 'PUT', path: '/settings/', response: updatedSettingsResponse([
                ...
                {key: 'stripe_connect_publishable_key', value: 'REDACTED_KEY'},
                {key: 'stripe_connect_secret_key', value: 'REDACTED_KEY'}
            ])}
        }});
    ...
    test('Supports updating Stripe Direct settings', async ({page}) => {
        const {lastApiRequests} = await mockApi({page, requests: {
            ...
            editSettings: {method: 'PUT', path: '/settings/', response: updatedSettingsResponse([
                {key: 'stripe_publishable_key', value: 'REDACTED_KEY'},
                {key: 'stripe_secret_key', value: 'REDACTED_KEY'}
            ])}
        }});
        ...
        const modal = page.getByTestId('stripe-modal');
        await modal.getByLabel('Publishable key').fill('REDACTED_KEY');
        await modal.getByLabel('Secure key').fill('REDACTED_KEY');
        await modal.getByRole('button', {name: 'Save Stripe settings'}).click();
